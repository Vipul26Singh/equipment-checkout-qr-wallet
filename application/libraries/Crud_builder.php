...
class Crud_builder
{
	...
	public function getFilterableField($withOption = false)
        {
                $fields = [];
                foreach ($this->crud as $contains)
                {
                        $field = array_keys($contains)[0];
                        if (isset($contains[$field]['show_in_column']) AND $contains[$field]['show_in_column'] AND isset($contains[$field]['field_filterable']) AND $contains[$field]['field_filterable']) {
                                if ($withOption) {
                                        $fields[$field] = $contains[$field];
                                } else {
                                        $fields[] = $field;
	...
	public function getExportableField($withOption = false)
        {
                $fields = [];
                foreach ($this->crud as $contains)
                {
                        $field = array_keys($contains)[0];
                        if (isset($contains[$field]['field_exportable']) AND $contains[$field]['field_exportable']) {
                                if ($withOption) {
                                        $fields[$field] = $contains[$field];
                                } else {
                                        $fields[] = $field;
