   ...
   <div class="row" >
      ...
      <div class="col-md-12">
         <div class="box box-warning">
            <div class="box-body ">
		     ...
		     <div class="row pull-right">
                        ...
                        <a class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', ['Blog']); ?>  (Ctrl+a)" href="<?=  site_url('administrator/blog/add'); ?>"><i class="fa fa-plus-square-o" ></i> <?= cclang('add_button'); ?></a>
                        <?php }) ?>
									<?php 
				$get_params = '';
			...
			?>
						 <?php is_allowed('blog_export', function() use ($get_params) {?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> Blog" href="<?= site_url('administrator/blog/export?'.$get_params); ?>"><i class="fa fa-file-excel-o" ></i> <?= cclang('export'); ?> XLS</a>
                        <?php }) ?>
                        <?php is_allowed('blog_export', function() use ($get_params) {?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> pdf Blog" href="<?= site_url('administrator/blog/export_pdf?'.$get_params); ?>"><i class="fa fa-file-pdf-o" ></i> <?= cclang('export'); ?> PDF</a>
			<?php }) ?>
			                     </div>
		...
		<div class="widget-user-header " id="advance_form_filter" style="display: none;">
			<form name="form_filter_blog" id="form_filter_blog" action="<?= base_url('administrator/blog/index'); ?>" method="get">
				<div class="col-sm-12">
																					<div class="col-sm-6">
                                                			<div class="form-group ">
									<label class="col-sm-4 control-label">Title</label>
                                                        		<div class="col-sm-8">
									<input type="text" class="form-control" name="title" value="<?= !empty($_GET['title']) ? $_GET['title']:' ' ?>"></input>
									</div>
									</div>
									</div>
																																												                                                                        <div class="col-sm-6">
				...
				<br>
					...
					<div class="col-sm-4">
						<a class="btn btn-danger btn-block btn-md btn-raised pull-right" value="Reset" href="<?= base_url('administrator/blog');?>" title="<?= cclang('reset_filter'); ?>">
                              ...
                              <?php if (!empty($blog->image)): ?>
                                ...
                                <?php else: ?>
                                  <a href="<?= BASE_URL . 'administrator/file/download/blog/' . $blog->image; ?>">
                                   <img src="<?= get_icon_file($blog->image); ?>" class="image-responsive image-icon" alt="image blog" title="image <?= $blog->image; ?>" width="40px"> 
                                 </a>
                                <?php endif; ?>
                              <?php endif; ?>
                           </td>
...
<script>
  $(document).ready(function(){
	...
	$('#sbtn').click(function(){
                var q = $('#filter').val();
                var f = $('#field').val();
                window.location.replace(BASE_URL + '/administrator/blog/index?q=' + q + '&f=' + f);
                return false; 
        });
