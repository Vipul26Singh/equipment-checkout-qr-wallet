...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class=""><a  href="{php_open_tag_echo} site_url('administrator/{table_name}?'.$query_string); {php_close_tag}"><?= ucwords(clean_snake_case($subject)); ?></a></li>
    ...
    <div class="row" >
        <div class="col-md-12">
			...
			<?php if(!empty($crud_nav_menu)) {
				...
				{php_close_tag}
                                <?php foreach($this->input->post("crud_nav_menu") as $val){
                                        if(!empty($val['nav_menu_name']) && !empty($val['show_add']) ) {
                                ?>
				<li >
			...
			{php_open_tag} 
				if(!empty($_GET['<?= $input ?>'])){
			{php_close_tag}
				<input type="hidden" name="<?= $input; ?>" id="<?= $input; ?>" value="{php_open_tag_echo} $_GET['<?= $input ?>'] {php_close_tag}" >
			 {php_open_tag}
				} else {
			{php_close_tag}
...
<script>
    $(document).ready(function(){
      ...
      $('.btn_save').click(function(){
		...
		redirect_forced = '';
		<?php if(!empty($redirect_add)) { ?>
			redirect_forced = BASE_URL + '<?=$redirect_add ?>';
			if(res.id){
			var red = redirect_forced.replace('{{id}}', res.id);
